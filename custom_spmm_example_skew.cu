/***************************************************************************
 * SpInfer 偏斜稀疏数据使用示例
 * 展示如何使用偏斜分布的稀疏矩阵进行SpMM计算和性能对比
 * 前90%行高稀疏度，后10%行低稀疏度（稠密）
 ***************************************************************************/
#include <iostream>
#include <vector>
#include <cuda_fp16.h>
...
int main(int argc, char** argv) {
    ...
    // 方式1: 从Python偏斜生成器加载自定义稀疏矩阵
    if (!load_dense_matrix_from_file("Input_date/skew_sparse_data_latest/skew_sparse_matrix.bin", A_h, M_GLOBAL, K_GLOBAL)) {
        printf("未找到偏斜稀疏矩阵文件，使用随机生成\n");
        generate_custom_sparse_matrix(A_h, M_GLOBAL, K_GLOBAL, sparsity_ratio);
    } else {
        printf("成功加载Python生成的偏斜稀疏矩阵\n");
    }
